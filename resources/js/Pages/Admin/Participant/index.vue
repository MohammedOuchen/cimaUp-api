<template lang="">
            ...
            <main class="flex-1 flex flex-col bg-gray-100 dark:bg-gray-700 transition duration-500 ease-in-out overflow-y-auto">
                <div class="mx-10 my-2">
                    <h2 class="my-4 text-4xl font-semibold dark:text-gray-400">
                        ...
                        <span>La liste des acteurs et réalisateurs</span>
                    </h2>
                ...
                <tbody>
                    <tr v-for="participant in this.participants" :key="participant.id">
                        <td class="border p-3">
                             {{ participant.id }}
                        </td>
                        <td class="border p-3">
                        ...
                        <td class="border p-3">
                            <div class="flex space-between">
                                <span @click="Edit(participant)" class="w-7 bg-yellow-500 text-gray-200 rounded hover:bg-yellow-400 px-1 py-1 focus:outline-none mx-1 cursor-pointer">
                                ...
                                <span @click="Delete(participant)" class="w-7 bg-red-500 text-gray-200 rounded hover:bg-red-400 px-1 py-1 focus:outline-none mx-1 cursor-pointer">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="fill-current text-white" viewBox="0 0 64 64">
    ...
    <jet-dialog-modal :show="this.ConfirmDeletion" @close="this.ConfirmDeletion = false">
        ...
        <template #content>
          Voulez-vous vraiment supprimer ce utilisateur? Une fois supprimé, toutes ses ressources et données seront définitivement supprimées.
        </template>
...
export default {
    ...
    methods:{
        Add(){
            this.$inertia.visit(route('admin.participant.create'))
        },
        Delete(data){
        ...
        deleteParticipant(){
                ...
                this.formDeleteParticipant.post(route('admin.participant.destroy', {id: this.participant_id}), {
                        preserveScroll: true,
                        onError: (error) => {
                        },
                        onSuccess: () => {
